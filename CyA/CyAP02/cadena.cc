...
/**
 * @brief Devuelve un lenguaje con todos los prefijos de una cadena
 * 
 * @return Lenguaje 
 */
Lenguaje Cadena::Prefijos() const {
  ...
  std::string prefijo;
  aux.InsertarCadena(Cadena("&", alfabeto_)); 
  for (int i = 0; i < GetSecuencia().length(); ++i) {
    prefijo += GetSecuencia()[i];  
...
/**
 * @brief Devuelve un lenguaje con todos los sufijos de una cadena
 * 
 * @return Lenguaje 
 */
Lenguaje Cadena::Sufijos() const {
  ...
  aux.InsertarCadena(Cadena("&", alfabeto_)); 
  for (int i = 0; i < GetSecuencia().length(); ++i) {
    std::string sufijo = GetSecuencia().substr(i);  
